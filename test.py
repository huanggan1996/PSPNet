...
@click.command()
@click.option('--data-path',type=str,default='E:/data/VOCdevkit/VOC2012',help='VOC data path')
@click.option('--image',type=str,default='./image/mot.jpg',help='input image')
@click.option('--snapshot',type=str,default='./models/PSPNet_29_0.1585.pth',help='model weight')
...
def test(data_path,image,snapshot,backend):
    ...
    misc.imsave('_'.join([image[:-4],'mask'])+'.png',decode_img)
    # misc.imsave('_'.join([image[:-4],'label2'])+'.png',pred)
    misc.toimage(pred,high=np.max(pred),low=np.min(pred)).save('_'.join([image[:-4],'label'])+'.png')
